...
import { BRANDS, HOTELS } from "../data";

function App() {
  const dispatch = useDispatch();
  const { view, hotels, brands, hotelFields, brandFields, filterByBrands } =
  ...
  useEffect(() => {
    // oneTimeData(); // stores data one time
  ...
  const onHotelChange = (e: FormEvent<HTMLInputElement | HTMLSelectElement>) =>
    dispatch(
      setHotelFields({
        ...hotelFields,
  ...
  const onBrandChange = (e: FormEvent<HTMLInputElement>) =>
    dispatch(
      setBrandFields({
        ...brandFields,
        [e.currentTarget.name]: e.currentTarget.value,
  ...
  const onSubmitBrand = () => {
    ...
    const brandIndex = brands.findIndex(
      (brand: brandFieldsProps) => brand.id === brandFields.id
    );

    if (isDuplicateBrand(brandIndex))
      return window.alert("Duplicate brand not allowed!");
  ...
  const onSubmitHotel = () => {
    ...
    if (!validStr(hotelFields.brandId))
      return alert(
        "Select a brand or click the plus icon on the header to create a brand!"
      );

    const hotelIndex = hotels.findIndex(
      (hotel: hotelFieldsProps) => hotel.id === hotelFields.id
    );
  ...
  const isDuplicateBrand = (index: number) =>
    brands[index]?.name.toLowerCase() ===
    brandFields?.name.toLowerCase().trim();
  ...
  const updateBrand = (findByIndex: number) => {
    // update a brand...
  ...
  const createHotel = () => {
    ...
    const newHotels = [
      ...hotels,
      { id, name, city, country, address, rating, brandId },
    ];
    dispatch(setHotels(newHotels));
  };

  const updateHotel = (findByIndex: number) => {
    // upate a hotel!
  ...
  const onDeleteBrand = (item: brandFieldsProps) => {
    ...
    const filterData = brands.filter(
      (brand: brandFieldsProps) => brand.id !== item.id
    );
  ...
  const onDeleteHotel = (item: hotelFieldsProps) => {
    if (!window.confirm("Are you sure you want to delete this?")) return;
    const filterData = hotels.filter(
      (hotel: hotelFieldsProps) => hotel.id !== item.id
    );
  ...
  const onFilterChange = async (values: string[]) =>
    dispatch(setFilterByBrands(values));

  const filterElements = (hotels: hotelFieldsProps[], filters: string[]) => {
    const newHotels = [...hotels];
    const brand = (id: string) =>
      brands.find((item: brandFieldsProps) => item.id === id);

    if (filters?.length)
      return newHotels
        .filter((hotel) => filters.includes(brand(hotel.brandId)?.name))
        .sort((a, b) => Number(b.rating) - Number(a.rating));
    return newHotels.sort(
      (a: hotelFieldsProps, b: hotelFieldsProps) =>
